...
NOTE: Program is functional, but need 2 API keys from cerebras to function. Fill them in in the apikeys file.
...
-   **API KEYS**
    All of this thought is done through Cerebras API. Only two separate API
    keys are needed,  since each module is called usually one at a time, and then there is an instance of two modules being called simultaneously, . These are stored in a
    separate file called apikeys. This text file simply has each
    API1 and API3 designation in brackets followed by the apikey for that.
...
- [ ] **Prompt Optimization:**
  - Fine-tune prompts for each module to achieve optimum output.
  - Check inner thoughts generated by each module to confirm they reflect the correct cognitive processes.

- [ ] **Compatibility and Speed:**
  - Make the system usable for local LLM as well as Cerebras.
...
- [ ] **User Interface Design:**
  - Develop an easy-to-use web GUI to allow users to interact with the system seamlessly.
